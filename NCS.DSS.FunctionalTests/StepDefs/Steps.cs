...
namespace FunctionalTests.StepDefs
{
    ...
    [Given(@"I post an Action with the following details:")]
        ...
        public bool ThenThereShouldBeARecordInTheChangeFeedTable(string table)
        {
            ...
            helper.SetConnection("Server=tcp:dfc-sit-shared-sql.database.windows.net, 1433; Initial Catalog=dss-test-shared-stag-db; Persist Security Info=False; User ID=SimonDavis; Password=nZx2GPx6d11koQ; MultipleActiveResultSets=False; Encrypt=True; TrustServerCertificate=False; Connection Timeout=30;");
            found = helper.DoesResourceExist(table, customerId);
